...
export function validateCatalog(input) {
  ...
  if (!legacy && (input.version !== 2 || input.currency !== 'USD')) throw new Error('El catálogo debe usar precios en USD.');
  ...
  const products = input.products.map((item) => {
    ...
    if (legacy && (!Number.isInteger(item.price) || item.price < 1)) throw new Error(`El precio de ${name} no es válido.`);
    const priceUsd = legacy ? Math.round(item.price / LEGACY_CUP_PER_USD * 100) / 100 : item.priceUsd;
    const cents = Math.round(priceUsd * 100);
    if (typeof priceUsd !== 'number' || !Number.isFinite(priceUsd) || cents < 1 || cents > 100_000_000 || Math.abs(priceUsd * 100 - cents) > 1e-7) throw new Error(`El precio USD de ${name} no es válido.`);
    ...
    return { id, name, category, priceUsd: cents / 100, badge, image: image.toString(), description, visible: item.visible };
  });
  return { version: 2, currency: 'USD', categories, products };
}

export async function readCatalog() {
  if (!process.env.GITHUB_TOKEN) return { catalog: validateCatalog(localCatalog), sha: null, configured: false };
